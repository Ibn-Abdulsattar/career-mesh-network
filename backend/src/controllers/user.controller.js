...
export const register = async (req, res, next) => {
  ...
  if (user) {
    if (user.isVerified) {
      return next(
        new ExpressError("User already exists. Please sign in.", 400),
      );
    }
  ...
  const subject = "Verify your Career Mesh account 🔐";

  const message = `
Hello ${user.username},

Thank you for joining Career Mesh! To complete your registration and verify your email address, please use the following One-Time Password (OTP):
...
export const verifyOtp = async (req, res, next) => {
  ...
  if (!email || !code) {
    return next(new ExpressError("Email and code are required", 400));
  }

  const newUser = await User.findOne({ where: { email } });
  if (!newUser) {
    return next(new ExpressError("User don't exist", 400));
  }
  ...
  if (!otpRecord || otpRecord.code !== code) {
    return next(new ExpressError("Invalid or expired code", 404));
  }
